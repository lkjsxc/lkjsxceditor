 ...
 */
 ...
 #define BUFCHUNK_COUNT 128      // Number of chunks in the pool (128 * 512 = 64 KiB)
 ...
 enum RESULT bufclient_init(struct bufclient* buf) {
     ...
     if (!buf->begin) return RESULT_ERR; // Failed to get initial chunk from pool
 ...
 enum RESULT bufclient_set_abs_pos(struct bufclient* buf, int target_pos) {
     ...
     if (target_pos < 0 || target_pos > buf->size) return RESULT_ERR;
     ...
     if (buf->size == 0 && target_pos == 0) {
         if (!buf->begin) { // If even the initial chunk isn't allocated (shouldn't happen after init)
             if (bufclient_init(buf) != RESULT_OK) return RESULT_ERR; // Try re-init
         }
 ...
 enum RESULT bufclient_insert_char(struct bufclient* buf, char c) {
     ...
     if (!new_chunk) return RESULT_ERR; // Out of memory (chunk pool exhausted)
     ...
     if (index == BUFCHUNK_SIZE) {
          ...
          if(chunk->next) chunk->next->prev = new_chunk; // Link back from following chunk
          else buf->rbegin = new_chunk; // Update buffer's end pointer if inserting after last chunk
     ...
     else buf->rbegin = new_chunk; // Update end pointer if splitting the last chunk
     new_chunk->prev = chunk;
     chunk->next = new_chunk;
 ...
 enum RESULT bufclient_delete_char(struct bufclient* buf) {
     ...
     if (index > 0) {
         ...
         if (chunk->size == 0 && !(chunk == buf->begin && chunk == buf->rbegin)) {
             ...
             if(prev_chunk) prev_chunk->next = next_chunk;
             else buf->begin = next_chunk; // Update buffer start pointer if first chunk removed
 
             if(next_chunk) next_chunk->prev = prev_chunk;
             else buf->rbegin = prev_chunk; // Update buffer end pointer if last chunk removed
     ...
     if (index == 0 && chunk != buf->begin) {
         ...
         if (!prev_chunk) return RESULT_ERR; // Should not happen in a consistent list
 
         // If previous chunk is already empty, something is wrong or delete has no effect
         if (prev_chunk->size == 0) return RESULT_ERR; // Cannot delete last char of empty chunk
         ...
         if (chunk->size == 0) {
             ...
             else buf->rbegin = prev_chunk; // Update buffer end if 'chunk' was the last one
 ...
 void enableRawMode() {
     ...
     if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw) == -1) die("tcsetattr failed on entry");
 ...
 int editorReadKey() {
     ...
     while ((nread = read(STDIN_FILENO, &c, 1)) != 1) {
         ...
         if (nread == -1 && errno != EAGAIN && errno != EINTR) die("read error");
     ...
     if (c == '\x1b') {
         ...
         if (seq[0] == '[') {
             if (seq[1] >= '0' && seq[1] <= '9') {
                  ...
                  if (read(STDIN_FILENO, &seq[2], 1) != 1) return '\x1b'; // Need final char
                  if (seq[2] == '~') {
                     switch (seq[1]) {
                         // Map standard xterm escape sequences
                         case '1': return HOME_KEY; // Often Home
                         case '3': return DEL_KEY;  // Often Delete
                         case '4': return END_KEY;   // Often End
                         case '5': return PAGE_UP;
                         case '6': return PAGE_DOWN;
                         case '7': return HOME_KEY; // Sometimes Home
                         case '8': return END_KEY;   // Sometimes End
                         default: return '\x1b'; // Unrecognized sequence number
                     }
                  } else {
                 ...
                 switch (seq[1]) {
                     ...
                     case 'H': return HOME_KEY; // Sometimes Home (vt100?)
                     case 'F': return END_KEY;   // Sometimes End (vt100?)
                     default: return '\x1b'; // Unrecognized char after '['
                 }
             }
         }
         // Check for VT100/Linux console sequences: ESC O ...
              ...
              switch (seq[1]) {
                 case 'H': return HOME_KEY; // Usually Home
                 case 'F': return END_KEY;   // Usually End
                 default: return '\x1b'; // Unrecognized char after 'O'
             }
         }
 ...
 int getCursorPosition(int *rows, int *cols) {
   ...
   while (i < sizeof(buf) - 1) {
     if (read(STDIN_FILENO, &buf[i], 1) != 1) break; // Error or timeout
     if (buf[i] == 'R') break; // End of response marker
     i++;
   }
   ...
   if (*ptr != ';') return -1; // Expect semicolon separator
   ptr++;
   while (*ptr >= '0' && *ptr <= '9') c = c * 10 + (*ptr++ - '0');
 ...
 int getWindowSize(int *rows, int *cols) {
     ...
     if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
         ...
         if (write(STDOUT_FILENO, "\x1b[999C\x1b[999B", 12) != 12) return -1; // Move cursor
 ...
 int find_line_start(int abs_pos) {
     if (abs_pos <= 0) return 0; // Already at or before start
     ...
     while (current_chunk && current_chunk_start_abs + current_chunk->size <= abs_pos) {
         ...
         for (int i = 0; i < current_chunk->size; i++) {
             if (current_chunk_start_abs + i >= abs_pos) break; // Don't scan past target
     ...
     if (line_start_pos > abs_pos) line_start_pos = abs_pos;
 ...
 int find_line_end(int abs_pos) {
     ...
     if (!current_chunk) return E.textbuf.size;
          ...
          case PAGE_UP:
             {
                 ...
                 int cy_actual = 0;
                 struct bufchunk* c = E.textbuf.begin; int p = 0;
                 while(c && p < current_abs_pos){ int end = (c==E.textbuf.cursor_chunk)?E.textbuf.cursor_index:c->size; for(int i=0;i<end && p<current_abs_pos; ++i){ if(c->data[i]=='\n') cy_actual++; p++;} if(p==current_abs_pos)break; if(c==E.textbuf.cursor_chunk && p + c->size > current_abs_pos) break; c=c->next;}
                 ...
                 // Adjust column similar to ARROW_UP logic
                  ...
                  int target_row = 0; c=E.textbuf.begin; p=0;
                  while(c && p < target_abs_pos){ int end=c->size; for(int i=0;i<end;++i){ if(p==target_abs_pos)goto end_row_count_pgup; if(c->data[i]=='\n')target_row++; p++;} c=c->next;} end_row_count_pgup:;
          ...
          case PAGE_DOWN:
             {
                 ...
                 while(lines_to_move-- > 0) {
                  ...
                  int target_row = 0; c=E.textbuf.begin; p=0;
                  while(c && p < target_abs_pos){ int end=c->size; for(int i=0;i<end;++i){ if(p==target_abs_pos)goto end_row_count_pgdn; if(c->data[i]=='\n')target_row++; p++;} c=c->next;} end_row_count_pgdn:;
 ...
 void editorScroll() {
     ...
     while(chunk && abs_pos < current_abs_pos) {
         ...
         for(int i = 0; i < end_idx; ++i) {
             if (abs_pos == current_abs_pos) goto end_scroll_calc; // Reached cursor position
                           ...
                           if (iscntrl(current_char) && current_char != '\t') {
                               ...
                               for (int s = 0; s < spaces_to_add; s++) {
                                   if (screen_col + s < E.screencols) {
                                       screenAppend(" ", 1);
                                   } else break; // Stop if space goes past screen edge
                               }
 ...
 void editorDrawStatusBar() {
     ...
     if (len < 0) len = 0; // Safety check for snprintf error
     if (len > E.screencols) len = E.screencols; // Truncate if too long
     ...
     while(chunk && abs_pos < current_abs_pos) {
         int end_idx = chunk->size;
         for(int i = 0; i < end_idx; ++i) {
            if (abs_pos == current_abs_pos) break; // Stop counting when cursor position reached
            if (chunk->data[i] == '\n') {
                current_line++; // Increment line count on newline
            }
             abs_pos++;
         }
         if (abs_pos == current_abs_pos) break;
         chunk = chunk->next;
     }
 ...
 void editorDrawMessageBar() {
     ...
     if (E.mode == MODE_COMMAND) {
         ...
         msglen = cmdlen;
         if (msglen > E.screencols) msglen = E.screencols; // Truncate if too long for screen
         if (msglen > 0) {
         ...
         if (msglen > E.screencols) msglen = E.screencols; // Truncate if too long
 ...
 void editorRefreshScreen() {
     ...
     if (E.mode == MODE_COMMAND) {
         ...
         while(c) { cmd_len += c->size; c = c->next; }
         if (cmd_len > E.screencols) cmd_len = E.screencols; // Clamp to screen width
 ...
 void editorProcessCommand() {
     ...
     if (cmdlen == 0) return; // Empty command does nothing
